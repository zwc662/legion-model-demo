...
def load_db(db_path, create_table_sql = None):
    if create_table_sql is not None:
        create_table(db_path, create_table_sql)
    conn = sqlite3.connect(db_path)
    schema = dump_db_json_schema(conn)
    conn.close()
    return schema
...
if __name__ == "__main__":
    ...
    for example in examples:
        ...
        #print(db_id, ':', create_table_sql, ':', question, ':', query)
        db_path = db_id + '.sqlite'
        schema = load_db(db_path, create_table_sql)
        output = run_query(db_path, query)
        print(question, query, output)
