...
* Docker - [Install Docker community edition](https://hub.docker.com/search/?type=edition&offering=community)
  * For MacOSX user with the latest Docker, run `sudo ln -s "$HOME/.docker/run/docker.sock" /var/run/docker.sock` after installation.
...
Find the current DOCKER ENDPOINT, e.g., `unix:///home/$username/.docker/run/docker.sock`. Replace `$username` with your true username and run the following in your shell:

```bash
DOCKER_HOST=unix:///home/username/.docker/run/docker.sock sam build
DOCKER_HOST=unix:///home/username/.docker/run/docker.sock sam deploy --guided
```
...
```bash
text2sql$ sam local invoke InferenceFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
text2sql$ sam local start-api
text2sql$ curl http://localhost:3000/classify_digit
```
...
```bash
text2sql$ sam logs -n InferenceFunction --stack-name "text2sql" --tail
```
...
```bash
sam delete --stack-name "text2sql"
```
